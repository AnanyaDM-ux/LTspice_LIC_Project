...
### Procedure :
1. Design the common-source amplifier circuit in LTspice following the given schematic.
2. Configure the resistor to 1KΩ, DC supply voltage to 1.8V, and gate voltage to 0.9V.
3. Download the library file tsmc018 (1).txt.
4. Create a dedicated folder and save both the library file and the LTspice project file in it.
5. Load the library file into LTspice using the .op Spice directive.
6. Assign the MOSFET model as CMOSN, as specified in the library file, with a channel length of 180nm and width of 1µm.
7. Determine the drain current based on the given power rating.
8. DC Analysis: In the simulation settings, select DC Sweep to obtain a range of current values from the circuit. The calculated and simulated current values should match, so adjust the MOSFET width accordingly, 
   as it is directly proportional to the drain current (Id) while keeping other parameters constant.
9. Transient Analysis: Modify the simulation settings from DC Sweep to Transient. Set the DC offset to 0.9V, amplitude to 50mV, and frequency to 1kHz. Set the stop time to 3ms and run the simulation to observe 
   the expected waveform.
10.AC Analysis: Change the simulation settings from Transient to AC Analysis. Choose the decade sweep type, set 20 points per decade, and define the frequency range from 0.1Hz to 1THz to generate the required AC 
   response.
...
2. The drain current (Id) is directly proportional to the MOSFET's width, keeping all other parameters constant.
3. DC analysis helps determine the operating point and verifies whether the MOSFET remains in saturation.
4. Transient analysis illustrates the MOSFET's response to a time-varying AC input (sine wave).
5. The circuit produces an amplified output, with a 180-degree phase shift between the input and output signals.
6. AC analysis provides insights into the gain and frequency response of the circuit.
   <br>
